...
(defun javamonkey-dir-to-package (pkgdir)
  (let* ((parts (mapcan
		 ...
		 (split-string pkgdir (char-to-string directory-sep-char))))
	 (pkg (mapconcat (lambda (s) s) parts ".")))
    pkg))

(defun javamonkey-package-to-dir (pkg)
  (let* ((parts (split-string pkg "\\."))
	 (slash (char-to-string directory-sep-char))
	 (dir (mapconcat (lambda (s) s) parts slash)))
    dir))
